...
class FoulMood:
    ...
    def __init__(self):
        self.bonus = 0
        self.foul_count = 0  # Renamed and repurposed from 'consecutive'

    def update(self, is_foul):
        """
        If the pitch is a foul, increment the foul_count for the at-bat.
        Then, update the bonus if the foul_count reaches the next threshold in BONUS_TIERS.
        Returns True if the bonus increased on this call.
        """
        if is_foul:
            self.foul_count += 1  # Always count the foul, no matter what came before.
            increased = False

            # Check the foul_count against thresholds.
            # For example: with 2 fouls, update bonus to 10 if not already at that tier;
            # with 3 fouls, update bonus to 25; 4 fouls → 50; 5 or more → 80.
            if self.foul_count == 2 and self.bonus < self.BONUS_TIERS[1]:
                self.bonus = self.BONUS_TIERS[1]
                increased = True
            elif self.foul_count == 3 and self.bonus < self.BONUS_TIERS[2]:
                self.bonus = self.BONUS_TIERS[2]
                increased = True
            elif self.foul_count == 4 and self.bonus < self.BONUS_TIERS[3]:
                self.bonus = self.BONUS_TIERS[3]
                increased = True
            elif self.foul_count >= 5 and self.bonus < self.BONUS_TIERS[4]:
                self.bonus = self.BONUS_TIERS[4]
                increased = True

            return increased
        else:
            # When the pitch is not a foul we simply return False.
            return False

    def get_bonus_message(self):
        """
        Return the bonus message corresponding to the current bonus.
        If not one of the preset tiers, display the current bonus.
        """
        return self.BONUS_MESSAGES.get(self.bonus, f"Foul Mood = {self.bonus}")

    def reset_per_atbat(self):
        """
        Reset the at-bat foul count but leave the persistent bonus intact.
        """
        self.foul_count = 0

    def reset(self):
        """
        Fully reset foul mood for a new game (both bonus and foul count).
        """
        self.bonus = 0
        self.foul_count = 0
...
# --- Bunt Attempt: for batters with low power and if a runner is on third base ---
    ...
    while strikes < 3 and balls < 4:
        ...
        if raw_roll == 75 and random.random() < 0.25:
            ...
            # (Optionally, you could branch here further as in your code—a 20% chance
            # to go immediately to beaned_walk, otherwise count it as a ball.)
        ...
        if roll >= 60:
            # The contact roll gets a boost from batter.batting.
            contact_roll = random.randint(1, 100) + batter.batting
            ...
            if batter.agility % 2 == 1:
                ...
                if contact_roll == triple_threshold:
                    if random.random() < 0.5:
                        # Fail this check: use base_bonus only.
                        pass
                    else:
                        # Succeed: simulate an extra bonus point.
                        base_bonus += 1
            ...
            if contact_roll >= effective_triple_threshold:
                ...
                # Otherwise, the ball is hit on the ground (line/ground out).
                foul_mood.update(False)
                current_outs += 1
                bso_display = format_bso(balls, strikes, current_outs)
                # Default ground-out message, with a chance for a flavor message if a tag occurs.
                ...
                for base_idx, runner in enumerate(base_runners):
                    if runner is not None:
                        if random.random() < extra_tag_chance:
                            ...
                            # Tagging a runner is an extra out.
                            current_outs += 1
                            bso_display = format_bso(balls, strikes, current_outs)
        ...
        if roll >= ball_threshold:
            ...
            # Otherwise, the outcome is a strike.
            # Split the strike type: 67% "looking," 33% "swinging."
